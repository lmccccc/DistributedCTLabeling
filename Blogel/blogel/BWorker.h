...
public:
    ...
    void active_vcompute()
    {
        ...
        for (BlockIter it = blocks.begin(); it != blocks.end(); it++)//each block
        {
            BlockT* block = *it;
            for (int i = block->begin; i < block->begin + block->size; i++)//each vertex
            {
                if (v_msgbufs[i].size() == 0)
                {
                    if (vertexes[i]->is_active())// no msg but active
                    {
#ifdef _DEBUG
                        cout << _my_rank <<  " v[" << i  << "] no msg but active" << endl;
#endif
                        block->activate(); //vertex activates its block
                        vertexes[i]->compute(v_msgbufs[i]);
                        AggregatorT* agg = (AggregatorT*)get_aggregator();//agg is empty
                        if (agg != NULL)
                ...
                {
#ifdef _DEBUG
                    cout << _my_rank <<  " v[" << i  << "] has msg" << endl;
#endif
                    block->activate(); //vertex activates its block
                    vertexes[i]->activate();
                    vertexes[i]->compute(v_msgbufs[i]);
    ...
    void all_vcompute()
    {
        ...
        for (BlockIter it = blocks.begin(); it != blocks.end(); it++)
        {
            ...
            {
                vertexes[i]->activate();
                vertexes[i]->compute(v_msgbufs[i]);//v compute, where does msg comes
                v_msgbufs[i].clear(); //clear used msgs
                AggregatorT* agg = (AggregatorT*)get_aggregator();
                if (agg != NULL)
                    agg->stepPartialV(vertexes[i]);
    ...
    void active_bcompute()
    {
        ...
        {
            if (b_msgbufs[i].size() == 0)// no msg
            {
                if (blocks[i]->is_active())// bug active
                {
#ifdef _DEBUG
                    cout << _my_rank <<  " b[" << i  << "] no msg but active" << endl;
#endif
                    blocks[i]->compute(b_msgbufs[i], vertexes);//dickstra
                    AggregatorT* agg = (AggregatorT*)get_aggregator();
                    if (agg != NULL)
            ...
            {
#ifdef _DEBUG
                cout << _my_rank <<  " b[" << i  << "] has msg" << endl;
#endif
                blocks[i]->activate();
                blocks[i]->compute(b_msgbufs[i], vertexes);//dickstra
...
//        hdfsDisconnect(fs);
    ...
    {
//        hdfsFS fs = getHdfsFS();
        BufferedWriter* writer = new BufferedWriter(outpath);
        for (BlockIter it = blocks.begin(); it != blocks.end(); it++)
        {
            BlockT* block = *it;
            for (int i = block->begin; i < block->begin + block->size; i++)
...
//        hdfsDisconnect(fs);
    ...
    void run(const WorkerParams& params)
    {
        ...
        //-------- create blocks ---------- create blocks from bid in vertexes
        int prev = -1;
        BlockT* block = NULL;
        int pos;
        ...
        StopTimer(WORKER_TIMER);
        PrintTimer("Block Initialization Time", WORKER_TIMER);
...
#ifdef _DEBUG
        cout <<  _my_rank << " start compute" << endl;
#endif
        if (compute_mode == VB_COMP)
        {
            while (true)
            {
...
#ifdef _DEBUG
                    if(_my_rank == MASTER_RANK)
                        cout << " active_vnum=" << active_vnum() << " active_bnum=" << active_bnum() << endl;
#endif
                ...
                {
#ifdef _DEBUG
                    cout << _my_rank << " v b all compute" << endl;
#endif
                    all_vcompute();
                    all_bcompute();
                }
...
#ifdef _DEBUG
                    cout <<  _my_rank << " v b active compute" << endl;
#endif
                    active_vcompute();//recv msg, update distance
                    active_bcompute();//dijkstra, send new distance
                }

#ifdef _DEBUG
                cout <<  _my_rank << " v b combine" << endl;
#endif
                vmessage_buffer->combine();
                bmessage_buffer->combine();

#ifdef _DEBUG
                cout <<  _my_rank << " v msg=" <<  vmessage_buffer->get_total_msg() << endl;
                cout <<  _my_rank << " b msg=" <<  bmessage_buffer->get_total_msg() << endl;
#endif
...
#ifdef _DEBUG
        cout << _my_rank << " run finfished" << endl;
#endif
    }
};
