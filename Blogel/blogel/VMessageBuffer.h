...
public:
    ...
    typedef typename VecsT::Vec Vec;// vector<pair<int, msg>>
    typedef typename VecsT::VecGroup VecGroup;
    typedef typename Map::iterator MapIter; //vector<vector<pair<int, msg>>>
    ...
    void combine()// v combine
    {
        //apply combiner
        Combiner<MessageT>* combiner = (Combiner<MessageT>*)get_combiner();//self defined combiner
#ifdef _DEBUG
        if(combiner == NULL) cout << "combiner is null" << endl;
#endif
        if (combiner != NULL)
#ifdef _DEBUG
            cout << "combiner executing" << endl;
#endif
