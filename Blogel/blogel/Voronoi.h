...
//        hdfsDisconnect(fs);
    ...
    void run(const WorkerParams& params, bool toUG)
    {
...
#ifdef _DEBUG
            cout << _my_rank << " scatter suc" << endl;
#endif
            //reading assigned splits (map)
            for (vector<string>::iterator it = assignedSplits.begin();
                 it != assignedSplits.end(); it++) {
...
#ifdef _DEBUG
        cout << _my_rank << " load graph suc" << endl;
#endif
        //send vertices according to hash_id (reduce)
        sync_graph();

#ifdef _DEBUG
        cout << " sync graph suc" << endl;
#endif
        ...
        while (true) {
            ...
            ResetTimer(4);
            //===================
            char bits_bor = all_bor(global_bor_bitmap);// bor: bit or
            active_vnum() = all_sum(active_count);
